 ...
 *
 */
public class Activity_Player extends YoukuBasePlayerActivity{
	//播放器控件
...
//		mYoukuPlayerView.initialize(this,pid);			//有pid的用户可以使用此接口配置pid参数
	...
	private void change(VideoQuality quality){
		try{
			...
			//接口详细信息可以参数使用文档
			int result = ApiManager.getInstance().changeVideoQuality(quality, Activity_Player.this);
			if(result == 0) Toast.makeText(Activity_Player.this, "不支持此清晰度", 2000).show();
		}catch(Exception e){
			Toast.makeText(Activity_Player.this, e.getMessage(), 2000).show();
		}
	}
